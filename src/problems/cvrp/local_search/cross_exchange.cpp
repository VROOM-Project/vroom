...
gain_t cross_exchange::compute_gain(const input& input,
  ...
  assert(sol[target_vehicle].size() >= 2);
  assert(source_rank < sol[source_vehicle].size() - 1);
  assert(target_rank < sol[target_vehicle].size() - 1);
  ...
  index_t s_after = input._jobs[sol[source_vehicle][source_rank + 1]].index();
  index_t t_current = input._jobs[sol[target_vehicle][target_rank]].index();
  index_t t_after = input._jobs[sol[target_vehicle][target_rank + 1]].index();
  index_t s_previous;
  if (source_rank == 0) {
  ...
  if (source_rank == sol[source_vehicle].size() - 2) {
    ...
    s_next = input._jobs[sol[source_vehicle][source_rank + 2]].index();
  }
  ...
  index_t t_next;
  if (target_rank == sol[target_vehicle].size() - 2) {
    assert(v_target.has_end());
    t_next = v_target.end.get().index();
  } else {
    t_next = input._jobs[sol[target_vehicle][target_rank + 2]].index();
  }
...
void cross_exchange::log() const {
  ...
  std::cout << "Cross_Exchange gain: " << gain << " - vehicle " << v_source.id
